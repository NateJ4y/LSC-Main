...
export default function App() {
  const [customizerState, setCustomizerState] = useState<CustomizerState>({
    ...
    embroideryOption: { enabled: true, text: 'HILUX 4X4', font: 'rugged', threadColor: '#d4af37' },
  ...
  const [cartItems, setCartItems] = useState<CartItem[]>([
    {
      ...
      priceZAR: 3950,
      quantity: 1,
      customDetails: { embroideryText: 'HILUX 4X4', consoleCoverIncluded: true }
    }
  ]);
  ...
  useEffect(() => {
    const checkAdminHash = () => {
      ...
      if (hash === 'admin' || window.location.pathname === '/admin') setActiveNav('admin');
    };
    checkAdminHash();
  ...
  const handleVehicleChange = (newVeh: VehicleSelection) => {
    setCustomizerState(prev => ({
      ...prev,
      vehicle: newVeh,
      embroideryOption: { ...prev.embroideryOption, text: prev.embroideryOption.enabled ? `${newVeh.model.split(' ')[0].toUpperCase()} 4X4` : '' }
    }));
  };

  const handleSelectMaterial = (materialId: string) => {
    const mat = MATERIALS_DATA.find(m => m.id === materialId) || MATERIALS_DATA[0];
    setCustomizerState(prev => ({ ...prev, materialId: mat.id, primaryColorId: mat.colors[0]?.id || 'canvas-charcoal' }));
  ...
  const handleSelectApplication = (appId: string) => {
    if (appId === '4x4-overland') {
      setCustomizerState(prev => ({ ...prev, materialId: 'heavy-duty-ripstop-canvas', primaryColorId: 'canvas-sand' }));
    } else if (appId === 'daily-suv') {
      setCustomizerState(prev => ({ ...prev, materialId: 'rhino-hide-leatherette', primaryColorId: 'rhino-onyx' }));
    } else if (appId === 'commercial-fleet') {
      setCustomizerState(prev => ({ ...prev, materialId: '600d-synthetic-polyester', primaryColorId: 'poly-charcoal' }));
    } else if (appId === 'luxury-executive') {
      setCustomizerState(prev => ({ ...prev, materialId: 'rhino-hide-leatherette', primaryColorId: 'rhino-cognac' }));
    }
    scrollToSection('customizer-studio');
    setActiveNav('customise');
  };
  ...
  const handleAddToCart = (newItem: CartItem) => {
    setCartItems(prev => {
      const existing = prev.find(i => i.title === newItem.title && i.subtitle === newItem.subtitle && i.vehicleSummary === newItem.vehicleSummary);
  ...
  const handleUpdateQuantity = (id: string, newQty: number) => {
    if (newQty <= 0) handleRemoveItem(id);
    else setCartItems(prev => prev.map(i => i.id === id ? { ...i, quantity: newQty } : i));
  };

  const handleRemoveItem = (id: string) => setCartItems(prev => prev.filter(i => i.id !== id));
  ...
  const handleNavSelection = (nav: string) => {
    setActiveNav(nav);
    if (nav === 'admin') {
      window.location.hash = 'admin'; window.scrollTo({ top: 0, behavior: 'smooth' }); return;
    }
    ...
    else if (nav === 'gallery') scrollToSection('gallery');
    else if (nav === 'fabrics' || nav === 'seat-covers') scrollToSection('fabric-matrix');
    else if (nav === 'vehicles') scrollToSection('vehicle-applications');
    else if (nav === 'customise') scrollToSection('customizer-studio');
    else if (nav === 'reviews') scrollToSection('reviews');
    else if (nav === 'quote') scrollToSection('quote-builder');
    else if (nav === 'contact') scrollToSection('contact-us');
  };
  ...
  if (activeNav === 'admin') return <AdminDashboard onBackToSite={() => handleNavSelection('home')} />;

  return (
    <div className="min-h-screen bg-[#0c0c0e] text-white flex flex-col font-sans selection:bg-orange-600 selection:text-white">
      <Header cartItems={cartItems} onOpenCart={() => setIsCartOpen(true)} onOpenSwatches={() => setIsSwatchesOpen(true)} onSelectNav={handleNavSelection} activeSection={activeNav} />

      <main className="flex-1">
        {/* 1. Hero: product first, immediate quote action */}
        <HeroVehicleSelector
          vehicle={customizerState.vehicle}
          onVehicleChange={handleVehicleChange}
          onStartConfiguring={() => { scrollToSection('customizer-studio'); setActiveNav('customise'); }}
          onViewGallery={() => { scrollToSection('gallery'); setActiveNav('gallery'); }}
        />

        {/* 2. Compact trust signals */}
        <TrustProofBar />

        {/* 3. Show the actual product early: less copy, more real fitments */}
        <RealFitmentGallery
          onStartQuote={() => { scrollToSection('quote-builder'); setActiveNav('quote'); }}
        />

        {/* 4. Visual material choices */}
        <MaterialMatrix onSelectMaterial={handleSelectMaterial} onOpenSwatches={() => setIsSwatchesOpen(true)} />

        {/* 5. Interactive product customisation */}
        <CustomizerStudio
        ...
        {/* 6. Short visual vehicle/application selector */}
        <VehicleApplications
          onSelectApplication={handleSelectApplication}
          onStartQuote={() => { scrollToSection('quote-builder'); setActiveNav('quote'); }}
        />

        {/* 7. Social proof — keep the section focused and human */}
        <CustomerReviews />

        {/* 8. Main conversion point */}
        <ComprehensiveQuoteSystem currentVehicle={customizerState.vehicle} />

        {/* 9. Workshop/contact details */}
        <ContactSection onStartQuote={() => { scrollToSection('quote-builder'); setActiveNav('quote'); }} />
      </main>
      ...
      <FreeSwatchModal isOpen={isSwatchesOpen} onClose={() => setIsSwatchesOpen(false)} />
